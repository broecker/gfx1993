...
namespace render
{
    ...
    // Describes the configuration, input/output and options that will be used to render primitives. This is passed in
    // explicitly to the various rasterization methods.
    struct RenderConfig
    {
        // The output buffers. At least either a framebuffer or a depthbuffer must be set. If both are set, they must
        ...
        // Checks that we have at least a single render target and a viewport and that the dimensions match.
