...
void testClipperTriangleSinglePlaneSinglePointInside() {
  ...
  assert(clipped[0].c.clipPosition ==
...
int testClipperTriangleSinglePlaneTwoPointsInside() {
  ...
  assert(clipped[0].b.clipPosition ==
         vec4(-2, -1, 0, 1)); // Bottom left unchanged.
  assert(clipped[0].c.clipPosition ==
         vec4(1, 0, 0, 1)); // Right position newly interpolated.
