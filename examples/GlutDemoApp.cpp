...
GlutDemoApp::GlutDemoApp(const std::string &name)
    : name(name), width(WIDTH_VGA), height(HEIGHT_VGA), texture(0),
      logFrameTime(true) {
...
void GlutDemoApp::glutDisplay() {
  ...
  // if (appInstance->renderTarget.framebuffer)
  {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, appInstance->width,
                 appInstance->height, 0, GL_RGBA, GL_FLOAT,
...
void GlutDemoApp::glutMotion(int x, int y) { appInstance->handleMotion(x, y); }
