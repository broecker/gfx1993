  ...
  WireSphere(float radius) {
    ...
    for (int theta = -80; theta < 90; theta += 10) {
      ...
      for (int phi = 0; phi <= 360; phi += 10) {
        glm::vec3 pos =
            glm::euclidean(glm::radians(glm::vec2(theta, phi))) * radius;
    ...
    // Note, this creates a giant 'tunnel' through the sphere's center. Ideally,
    // we want to draw all the line loops in multiple draw calls.
  }
};
...
protected:
  ...
  void renderFrame() override {
    ...
    try {
      rasterizer->drawLineStrip(renderConfig, sphere->getVertices(),
                                sphere->getIndices());
